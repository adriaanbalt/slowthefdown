...
gulp.task('rev-iconfont-workaround', ['rev-assets'], () => {
  var manifest = require(path.join(config.root,'public', 'rev-manifest.json'));
  var fontList = [];
  ...
  var streams = fontList.map(function(file) {
    ...
    return gulp.src(path.join(config.root,'public', file.path + '*.!(svg)'))
      .pipe(rename({suffix: file.hash}))
      .pipe(gulp.dest(iconConfig.dest));
  });

  // Re-write rev-manifest.json to disk
  fs.writeFile(path.join(config.root,'public', 'rev-manifest.json'), JSON.stringify(manifest, null, 2));
