...
// From three.js
// Modified by Andrew Best (anrew@blamsoft.com)
  ...
  parse = function(text) {
    function parseNode(node, style) {
      ...
      switch (node.nodeName) {
        ...
        case 'path':
          style = parseStyle(node, style);
          if (node.hasAttribute('d') && isVisible(style))
    ...
    console.log('SVGLoader: new DOMParser');
    const parser = new DOMParser();

    console.log('SVGLoader: Parsing');
    var xmlDoc = parser.parseFromString(text);

    parseNode(xmlDoc.documentElement, { fill: '#000' });
    console.log('SVGLoader: Parsed');
