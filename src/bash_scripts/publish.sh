...
jq '.version = '\"$VERSION\"'' temp.json > package.json

rm temp.json

echo
echo === Publishing Project Version: ${VERSION} from branch $BRANCH on release channel $CHANNEL===
echo

expo publish --release-channel $CHANNEL --config app.json
