...
THREE.SVGLoader.prototype = {
	...
	parse: function (text) {
		...
		function parseArcCommand(
			...
			var x1p =
				Math.cos(x_axis_rotation) * dx2 +
				Math.sin(x_axis_rotation) * dy2;
			var y1p =
				-Math.sin(x_axis_rotation) * dx2 +
				Math.cos(x_axis_rotation) * dy2;
		...
		function parseNodeTransform(node) {
			...
			for (
				var tIndex = transformsTexts.length - 1;
				tIndex >= 0;
				tIndex--
			) {
...
THREE.SVGLoader.pointsToStroke = function (
	...
	if (
		THREE.SVGLoader.pointsToStrokeWithBuffers(
			points,
			style,
			arcDivisions,
			minDistance,
			...
			if (nextPoint !== undefined) {
				...
				if (dot !== 0) {
					...
					switch (style.strokeLineJoin) {
						...
						default:
							var miterFraction =
								(strokeWidth2 * style.strokeMiterLimit) /
								miterLength2;
